...
UPDATE_TARGET_EVERY = 90
MODEL_NAME = f"ONE_AGENT_REPLAY{REPLAY_MEMORY_SIZE}_3X64_32D_MSE"

MIN_REWARD = -1

EPISODES = 6000 #100
used_loss = Huber(delta=1.0)

DISCOUNT = 0.99 #0.99
epsilon = 1
EPSILON_DECAY = 0.9988 ## 0.9975 99975
MIN_EPSILON = 0.001
...
class CarEnv:
    ...
    def process_lane_data(self, event):
        if event.crossed_lane_markings[0].type not in ['NONE', 'Broken']:
    ...
    def step(self, action):
        if action == 0:
            self.vehicle.apply_control(carla.VehicleControl(throttle=1.0*self.THROTTLE_AMT, steer=-1*self.STEER_AMT))
        elif action == 1:
            self.vehicle.apply_control(carla.VehicleControl(throttle=1.0, steer=0))
        elif action == 2:
            self.vehicle.apply_control(carla.VehicleControl(throttle=1.0*self.THROTTLE_AMT, steer=1*self.STEER_AMT))
        elif action == 3:
            self.vehicle.apply_control(carla.VehicleControl(brake=1*self.BRAKE_AMT))
        ...
        if len(self.collision_hist) != 0:
            ...
            # Apply sigmoid to collision in order to limit extreme collisions
        ...
        # SI SE MUEVE:
            ...
            reward = 1-((abs(kmh-30))/100)

        elif abs(kmh-30)/100<=1:
...
class DQNAgent:
    ...
    def train(self):
        ...
        history = self.model.fit(np.array(X)/255, np.array(y), batch_size=TRAINING_BATCH_SIZE, verbose=1, shuffle=False, callbacks=[self.tensorboard] if log_this_step else None)
        agent.history.append(history.history)
    ...
    def train_in_loop(self):
        ...
        # AQUÍ AHORA ES 4 PORQUE LA PREDICCIÓN ES SOBRE EL Q-VALUE DE LAS 4 ACCIONES POSIBLES
        y = np.random.uniform(size=(1, 4)).astype(np.float32)
        # ANTES ERA: y = np.random.uniform(size=(1, 3)).astype(np.float32)
...
if __name__ == '__main__':
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        #try:
            ...
            if not episode % AGGREGATE_STATS_EVERY or episode == 1:
                ...
                df.to_csv(f'results/{MODEL_NAME}/results_{MODEL_NAME}_lr_0.0001_ep_decay_{EPSILON_DECAY}_discount_{DISCOUNT}_UPDATE_CADA_30.csv')
                # Save model, but only when min reward is greater or equal a set value
                if min_reward >= MIN_REWARD:
                    agent.model.save(f'results/{MODEL_NAME}/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.h5')
                print(agent.history)
    ...
    trainer_thread.join()
    agent.model.save(f'results/{MODEL_NAME}/{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(time.time())}.hdf5')
